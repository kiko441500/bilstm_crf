...
def get_model(token_num,#字典长度
              pos_num=512,#最大句子位置长
              seq_len=512,#最大句子长
              embed_dim=768,#嵌入维度
              transformer_num=12,#transformer数量
              head_num=12,#每个transformer中的头数量
              feed_forward_dim=3072,#前向反馈的维度
              dropout_rate=0.1,
              attention_activation=None,#注意力激活函数
              feed_forward_activation='gelu',#前行反馈激活函数
              training=True,
              trainable=None,
              output_layer_num=1,#输出神经元为1
    ...
    inputs = get_inputs(seq_len=seq_len)#返回一个list
