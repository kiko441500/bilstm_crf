...
def build_model_from_config(config_file,
    ...
    if trainable is None:
        trainable = training
    model = get_model(
        token_num=config['vocab_size'],#21128
        pos_num=config['max_position_embeddings'],#512
        seq_len=seq_len,
...
def load_trained_model_from_checkpoint(config_file,
    ...
    :param checkpoint_file: The path to the checkpoint files, should end with '.ckpt'.
    :param training: If training, the whole model will be returned.
                     Otherwise, the MLM and NSP parts will be ignored.(masked lm, next sentence prediction)
